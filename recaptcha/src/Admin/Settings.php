...
class Settings {
	...
	public function __construct(\CD\recaptcha\Config $config) {
		...
		$this->option_name = $this->page_slug; // Keep this value different from Config::option_name
			...
			'thresholds' => [
				...
				'section_callback' => function() {
					printf('<p class="hidden show-field-for-v3 show-field-for-ent_score">%s</p>',
						...
						sprintf(__( 'reCAPTCHA returns a score, where %1$s is very likely a good interaction and %2$s is very likely a bot.', 'cd-recaptcha' ), 
 ...
 */
		...
		$fields = [
			...
			'recaptcha_version' => [
				...
				'options'       => [
					'ent_score'        => sprintf('[%s] %s', __( 'Enterprise', 'cd-recaptcha' ), __( 'Score', 'cd-recaptcha' )),
					'ent_checkbox'     => sprintf('[%s] %s', __( 'Enterprise', 'cd-recaptcha' ),__( 'Checkbox', 'cd-recaptcha' )),
					'ent_invisible'    => sprintf('[%s] %s', __( 'Enterprise', 'cd-recaptcha' ),__( 'Invisible', 'cd-recaptcha' )),
					'ent_policy_based' => sprintf('[%s] %s', __( 'Enterprise', 'cd-recaptcha' ),__( 'Policy-based', 'cd-recaptcha' )),
					'v2_checkbox'      => sprintf('[%s] %s', __( 'Legacy', 'cd-recaptcha' ), __( 'v2 Checkbox', 'cd-recaptcha' ) ),
					'v2_invisible'     => sprintf('[%s] %s', __( 'Legacy', 'cd-recaptcha' ), __( 'v2 Invisible', 'cd-recaptcha' ) ),
					'v3'               => sprintf('[%s] %s', __( 'Legacy', 'cd-recaptcha' ), __( 'v3', 'cd-recaptcha' ) ),
				],
				'desc'          => sprintf(
			...
			'gc_api_key' => [
				...
				'field_class'   => 'regular-text',
				'desc'          => sprintf(
					__( 'It\'s recommended to set up the API key in such way that it only has access to "reCAPTCHA Enterprise API". Do not have a key that can access everything! %s' , 'cd-recaptcha' ),
			...
			'theme' => [
				...
				'desc'          => sprintf(
					__( 'Color theme of the widget. Select %s to set the theme based on the brightness of the page\'s background color.', 'cd-recaptcha' ),
			...
			'badge' => [
				...
				'desc'          => sprintf(
					__( 'Position of the widget. Select %s to place the widget based on text direction (on a page with "right-to-left", the placement will be on the left).', 'cd-recaptcha' ),
					sprintf( '<i>%s</i>', __( 'Automatic', 'cd-recaptcha' ) )
				),
			],
			...
			'submit_user_email' => [
				'label'         => __( 'User\'s e-mail address', 'cd-recaptcha' ),
				'section_id'    => 'data_submisisons',
				'type'          => 'checkbox',
				'class'         => 'hidden show-field-for-ent_score show-field-for-ent_policy_based show-field-for-ent_checkbox show-field-for-ent_invisible',
			],
			'submit_user_registered' => [
				'label'         => __( 'User\'s registration date', 'cd-recaptcha' ),
				'section_id'    => 'data_submisisons',
				'type'          => 'checkbox',
				'class'         => 'hidden show-field-for-ent_score show-field-for-ent_policy_based show-field-for-ent_checkbox show-field-for-ent_invisible',
			],
			...
			$log_fields = [
				...
				'debug_log_min_level' => [ 
					...
					'options'       => [
						...
						sprintf( '%d - %s', 1, __( 'Error', 'cd-recaptcha' ) ),
						sprintf( '%d - %s', 2, __( 'Warning', 'cd-recaptcha' ) ),
						sprintf( '%d - %s', 3, __( 'Notice', 'cd-recaptcha' ) ),
						sprintf( '%d - %s', 4, __( 'Info', 'cd-recaptcha' ) ),
						sprintf( '%d - %s', 5, __( 'Debug', 'cd-recaptcha' ) ),
					],
